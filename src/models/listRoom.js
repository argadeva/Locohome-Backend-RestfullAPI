...
module.exports = {
  ...
  searchlistRoom: (data, date, page) => {
    ...
    return new Promise((resolve, reject) => {
      connecting.query(
        `SELECT listroom.*, room.id as idRoom, room.homeName, room.phoneNumber, room.ownerName, room.detailAddress, room.provinsi, room.kotaKabupaten, room.kecamatan, room.kecamatan, room.long, room.lat, GROUP_CONCAT(imagelistroom.image) FROM listroom INNER JOIN imagelistroom ON listroom.id = imagelistroom.idRoom INNER JOIN room ON listroom.idRoom = room.id WHERE listroom.id NOT IN (SELECT orders.idListRoom FROM orders WHERE orders.dateCheckIn BETWEEN '${date.dateCheckIn}' AND '${date.dateCheckOut}' OR (orders.dateCheckOut - INTERVAL 1 DAY) BETWEEN '${date.dateCheckIn}' AND '${date.dateCheckOut}' OR '${date.dateCheckIn}' BETWEEN orders.dateCheckIn AND (orders.dateCheckOut - INTERVAL 1 DAY) OR '${date.dateCheckOut}' BETWEEN orders.dateCheckIn AND (orders.dateCheckOut - INTERVAL 1 DAY)) AND (room.homeName LIKE '%${data}%' OR room.provinsi LIKE '%${data}%' OR room.kotaKabupaten LIKE '%${data}%' OR room.kecamatan LIKE '%${data}%') AND listroom.id = imagelistroom.idRoom GROUP BY listroom.id ORDER BY ${date.sort} ASC LIMIT ${total}, ${display}`,
        (err, result) => {
          if (!err) {
            resolve(result);
          } else {
