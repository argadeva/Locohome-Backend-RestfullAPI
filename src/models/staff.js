...
module.exports={
    ...
    updateStaff: (id_staff, data) => {
        return new Promise((resolve, reject) => {
            connection.query("UPDATE staff SET ? WHERE id = ?",[data, id_staff], (err, result) => {
                if(!err){
                    resolve(result);
                }else{
                    reject(new Error(err));
    ...
    deleteStaff: (id_staff) => {
        return new Promise((resolve, reject) => {
            connection.query("DELETE FROM staff WHERE id=?", id_staff, (err, result) => {
                if(!err){
                    resolve(result);
                }else{
                    reject(new Error(err));
    ...
    loginStaff: (email, password, token) =>{
        return new Promise((resolve, reject) => {
            ...
            connection.query("SELECT * FROM staff WHERE email=? AND password = ?", [email, password], (err, result) => {
                if(!err){
                    connection.query(`UPDATE staff set token='${token}' where email='${email}'`)
                    resolve(result);
                }else{
                    reject(new Error(err));
                }
    ...
    verificationStaff: token => {
      ...
      logoutStaff: (token) => {
        return new Promise((resolve, reject) => {
            connection.query("UPDATE staff SET token = null WHERE token = ?",[ token], (err, result) => {
                if(!err){
                    resolve(result);
                }else{
                    reject(new Error(err));
    ...
    checkingToken: (token) => {
        return new Promise((resolve, reject) => {
            connection.query("SELECT token FROM staff WHERE token = ?",[ token], (err, result) => {
                if(!err){
                    resolve(result);
                }else{
                    reject(new Error(err));
